...
class Trader(object):
    ...
    def buy(self, timestamp, entity_ids, ignore_in_position=True):
        ...
        if entity_ids:
            ...
            for entity_id in entity_ids:
                # [架构重点：发号施令]
                # 这里并未产生真实的交易 API 调用，而是针对每个决定要买的股票，
                # 生成了一个带有 position_pct (拟动用资金百分比) 的 TradingSignal，
                # 并存入内存队列 trading_signals 中，等待下一次循环周期才会执行。
    ...
    def run(self):
        ...
        for timestamp in self.entity_schema.get_interval_timestamps(
            ...
            # [主循环 步骤1：每日开盘广播]
            # 通知所有的监听器（比如虚拟账户在这个时候就该重置每日盈亏和处理昨日未完订单了）
            ...
            # [主循环 步骤2：执行【上一周期】遗留的交易信号]
            # 取出 self.trading_signals 中的买卖指令，通过 on_trading_signals 下发给真实柜台或模拟交易台。
            # 这是经典量化规则“信号隔夜执行”的体现：防止利用当下 K 线走完才确立的买点，骗自己买在当天的历史价格上。比如你的信号其实是一个周期走完后根据这个周期的k线算出来的，你的下单周期其实就不能是在这个周期
            ...
            # [主循环 步骤5：落锤，签发买卖令]
            # 把今天决定要买和卖的股票代码转化为 TradingSignal 信号包裹，压入等候队列中（它将在明天进入循环时在“步骤2”被执行）
            if short_selected:
            ...
            # [主循环 步骤6：每日收盘广播]
            # on_trading_close 通知各级账户结算今日总资产状况并进行对账核算
            if self.level >= IntervalLevel.LEVEL_1DAY or (
